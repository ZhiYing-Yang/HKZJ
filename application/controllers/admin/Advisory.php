...
class Advisory extends CI_Controller
{
  ...
  public function publish(){
    if(empty($this->input->post())){
      $this->load->view('admin/advisorys/publish_news.html');
    }else{
      $data = array(
        ...
        if($this->db->insert('advisory', $data)){
            get_json(200, '发布成功！');
        }else{
...
public function manage($type = 'id', $offset = 0){
  $where_arr = array();
  $per_page = 10;
  $page_url = site_url('admin/advisory/manage/'.$type);
  ...
  $data['news'] = $this->advisory->get_news_list($where_arr, $offset, $per_page,$type);
  $this->load->view('admin/advisorys/news.html',$data);
}
//搜索
public function news_search($keywords){
    $data['keywords'] = urldecode($keywords);
    $data['news'] = $this->advisory->get_news_search($keywords);
    $data['link'] = '';
    $this->load->view('admin/advisorys/news.html', $data);
}
...
public function news_action($action, $id){
    $news = $this->advisory->get_news_list(array('id'=>$id),0,1);
    if(empty($news)){
        alert_msg('该信息已被删除！');
    }
    $new = $new[0];
    ...
    if($action == 'delete'){  //删除
        $msg = '删除';
        if($this->db->delete('advisory', array('id'=>$id))){
            $status = true;
        }
    }
...
public function edit($id){
  $data = $this->advisory->get_news_list(array('id'=>$id),0,1)[0];
  $this->load->view('admin/advisorys/edit.html',$data);
}
}
