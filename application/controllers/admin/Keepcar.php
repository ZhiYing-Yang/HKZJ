...
class Keepcar extends CI_Controller
{
  ...
  public function publish(){
    if(empty($this->input->post())){
      $this->load->view('admin/keepcar/publish_news.html');
    }else{
      $data = array(
        ...
        if($this->db->insert('keepcars', $data)){
            get_json(200, '发布成功！');
        }else{
...
public function index($type = 'id', $offset = 0){
  $where_arr = array();
  $per_page = 10;
  $page_url = site_url('admin/keepcar/manage/'.$type);
  $total_rows = $this->db->count_all_results('keepcars');
  $offset_uri_segment = 5;
  $this->load->library('myclass');
  $data['link'] = $this->myclass->fenye($page_url, $total_rows, $offset_uri_segment, $per_page);
  $data['news'] = $this->keepcar->get_news_list($where_arr, $offset, $per_page,$type);
  $this->load->view('admin/keepcar/news.html',$data);
}
//搜索新闻
public function news_search($keywords){
    $data['keywords'] = urldecode($keywords);
    $data['news'] = $this->keepcar->get_news_search($keywords);
    $data['link'] = '';
    $this->load->view('admin/keepcar/news.html', $data);
}
...
public function news_action($action, $id){
    $news = $this->keepcar->get_news_list(array('id'=>$id),0,1);
    if(empty($news)){
        alert_msg('该信息已被删除！');
    }
    $new = $new[0];
    ...
    if($action == 'delete'){  //删除
        $msg = '删除';
        if($this->db->delete('keepcars', array('id'=>$id))){
            $status = true;
        }
    }
...
public function edit($id){
  $data = $this->keepcar->get_news_list(array('id'=>$id),0,1)[0];
  $this->load->view('admin/keepcar/edit.html',$data);
}
}
