...
class Wechat_device extends CI_Wechat_common {
    ...
    public function applyStatusShakeAroundDevice($apply_id)
    {
        ...
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::SHAKEAROUND_DEVICE_APPLYSTATUS . "access_token={$this->access_token}", self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
