...
class Wechat_poi extends CI_Wechat_common {
    ...
    public function addPoi($data)
    {
        ...
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::POI_ADD . "access_token={$this->access_token}", self::json_encode($data));
        if ($result) {
    ...
    public function delPoi($poi_id)
    {
        ...
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::POI_DELETE . "access_token={$this->access_token}", self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (empty($json) || !empty($json['errcode'])) {
                $this->errCode = isset($json['errcode']) ? $json['errcode'] : '505';
    ...
    public function updatePoi($data)
    {
        ...
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::POI_UPDATE . "access_token={$this->access_token}", self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (empty($json) || !empty($json['errcode'])) {
                $this->errCode = isset($json['errcode']) ? $json['errcode'] : '505';
    ...
    public function getPoi($poi_id)
    {
        ...
        $data = array('poi_id' => $poi_id);
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::POI_GET . "access_token={$this->access_token}", self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (empty($json) || !empty($json['errcode'])) {
                $this->errCode = isset($json['errcode']) ? $json['errcode'] : '505';
    ...
    public function getCategory()
    {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . self::POI_CATEGORY . "access_token={$this->access_token}");
        if ($result) {
            $json = json_decode($result, true);
            if (empty($json) || !empty($json['errcode'])) {
                $this->errCode = isset($json['errcode']) ? $json['errcode'] : '505';
