    ...
    $.fn.extend({
            ...
            try {
                $(_this._opt.imgTar).on('change', function(e) {
                    ...
                    reader.onload = function(f) {
                        ...
                        console.log('图片大小' + file.size + '。  Math.ceil后的大小' + Math.ceil(file.size / 1024 / 1024));
                        if (_this._opt.compressSize && Math.ceil(file.size / 1024 / 1024) > _this._opt.compressSize) {
                            console.log('规定图片大小' + _this._opt.compressSize);
                            var quality = Math.ceil(file.size / 1024 / 1024)/10-0.1; //图片压缩质量
                            // 解决Firefox读取不到图片高、宽
                            setTimeout(function() {
                                data = _this.compressHandler(img, quality);
                                console.log('执行了压缩');
        ...
        compressHandler: function(img, quality) {
            console.log('执行了压缩函数');
        ...
        upload: function(data) {
            var _this = this,
                filed = _this._opt.uploadField || 'uploadfile';
